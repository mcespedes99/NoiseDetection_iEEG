...
def main(model_name: str, dir_save: str, processes: int, features: str, srate: int):
    ...
    assert features in ['CWT','Hilbert'] 
    ...
    ## PARAMETERS
    n_classes = 3
    input_size = 1 # Number of images per example
    input_length_map = {
        'CWT': {
            2048: 6094},
        'Hilbert': {
    ...
    input_length = input_length_map[features][srate]
    ...
    # df_path = '/scratch/mcesped/Datasets/segments_mayo_fnusa_complete_version.csv'
    df_train_path = '/scratch/mcesped/Datasets/Multiclass/df_train.csv'
    df_val_path = '/scratch/mcesped/Datasets/Multiclass/df_val.csv'

    train_transform = transforms.Compose(
        [
            #transforms.Resize((30, 100), antialias=True),
            transforms.RandomHorizontalFlip(p=0.3),
            utils.RandomMask(p=0.3, f_max_size= 1, t_max_size = int(0.1*input_length), n_masks_f=1, n_masks_t=3),
        ]
    )
    val_transform = (
